...
def plot_success_probability_heatmap(df, num_bins_y=18, num_bins_z=6):
    ...
    Parámetros:
    df : DataFrame
        DataFrame con las columnas ['y_end_fixed', 'z_end_fixed', 'model_proba'].
    num_bins_y : int
    ...
    df['y_bin'] = pd.cut(df['y_end_fixed'], bins=num_bins_y, labels=False)
    df['z_bin'] = pd.cut(df['z_end_fixed'], bins=num_bins_z, labels=False)
...
def plot_interpolated_probability_contour(df, num_bins_y=18, num_bins_z=6):
    ...
    y_centers = df.groupby('y_bin')['y_end_fixed'].mean().values
    z_centers = df.groupby('z_bin')['z_end_fixed'].mean().values
    ...
    y_fine = np.linspace(df['y_end_fixed'].min(), df['y_end_fixed'].max(), 100)
    z_fine = np.linspace(df['z_end_fixed'].min(), df['z_end_fixed'].max(), 100)
...
def plot_goal_percentage_heatmap(df, bins_y, bins_z, cmap_color="Greens"):
    ...
    Parámetros:
    df : DataFrame
        DataFrame con las columnas ['y_end', 'z_end_fixed', 'NaEventType'].
    bins_y : int
        Número de divisiones en Y (ancho del arco).
    bins_z : int
        Número de divisiones en Z (altura del arco).
    ...
    """
    # Discretizar las coordenadas en cuadrantes
    df['y_bin'] = pd.cut(df['y_end_fixed'], bins=bins_y, labels=False)
    df['z_bin'] = pd.cut(df['z_end_fixed'], bins=bins_z, labels=False)
